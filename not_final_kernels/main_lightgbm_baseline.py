...
def main():
    ...
    class MarketNewsLinker(object):
        ...
        def __init__(self, concatable_fields):
            self.market_df = None
            self.news_df = None
            self.link_df = None
    ...
    gc.collect()
    linker = MarketNewsLinker(Features.CONCATABLE_FEATURES)

    # In[ ]:
    market_train_df = linker.link(market_train_df, news_train_df)
    ...
    X, valid_X, market_obs_ids, feature_names = to_train_dataset(market_train_df, features.news, news_feature_names,
                                                                 train_size=0.8)
    ...
    for (market_obs_df, news_obs_df, predictions_template_df) in tqdm(days):
        make_predictions(MARKET_ID, NEWS_ID, linker, pipeline, news_feature_names, model, market_obs_df, news_obs_df,
...
def make_predictions(MARKET_ID, NEWS_ID, linker, pipeline, news_feature_names, model, market_obs_df, news_obs_df,
                     predictions_df):
...
def load_train_dfs():
    try:
        ...
        market_train_df = pd.read_csv(TEST_MARKET_DATA, encoding="utf-8")
        news_train_df = pd.read_csv(TEST_NEWS_DATA, encoding="utf-8")
        env = None
...
class TwoSigmaDataLoader(DataLoader):
...
class BaseMLPTrainer(object):
    ...
    def save_models(self):
        torch.save(self.model.state_dict(), "twosigma_{}.model".format(self._current_epoch))
...
class Preprocess(ColumnTransformer):

    def __init__(self, sparse_threshold=0.3, n_jobs=None, transformer_weights=None):
        transformers = [
            (col, LogTransformer(), col) for col in self.get_log_normal_columns()
            # (col, TahnEstimators(), col) for col in self.get_columns_scaled()
        ]
        super().__init__(self.transformers, remainder='passthrough', sparse_threshold=sparse_threshold,
...
class MarketPreprocess(Preprocess):
    ...
    def __init__(self, sparse_threshold=0.3, n_jobs=None, transformer_weights=None):
        super().__init__(sparse_threshold=sparse_threshold,
                         n_jobs=n_jobs, transformer_weights=transformer_weights)
...
class LogTransformer(FunctionTransformer):
    ...
    def __init__(self,
        ...
        accept_sparse = False
        pass_y = 'deprecated'
        super().__init__(func, inverse_func, validate, accept_sparse, pass_y, kw_args, inv_kw_args)
...
class TahnEstimators(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, copy=None):
        return 0.5 * (np.tanh(0.01 * (X - self.mean_) / self.std_ + 1))
