...
class ModelWrapper(ABC):
    ...
    def split_train_validation(market_obs_ids, train_X, train_Y, train_size, train_X2=None):
        ...
        if train_X2 is not None and len(train_X2) > 0:
...
def is_not_empty(list_like):
    return list_like is not None and len(list_like) > 0
...
def make_predictions(MARKET_ID, NEWS_ID, linker, market_preprocess, news_preprocess, pipeline, news_feature_names,
    ...
    predict_indices = [market_id - 1 for market_id in market_train_obs_ids if market_id >= predict_id_start]
...
def create_data_loader(matrix: np.ndarray, labels: np.ndarray, batch_size: int, shuffle: bool):
    def transformers(item):
        return torch.from_numpy(item.todense().astype("float32"))

    dataset = TorchDataset(matrix, labels.reshape((-1, 1)), transformers)
...
class MLPWrapper(ModelWrapper):
    ...
    def create_dataset(self, market_train, features, news_feature_names, train_batch_size, valid_batch_size):
        ...
        self.valid_data_loader = create_data_loader(valid_matrix, valid_matrix.astype(""), valid_labels,
...
class LagAggregationTransformer(DfTransformer):
    ...
    @measure_time
    def transform(self, df: pd.DataFrame, n_pool=8):
        ...
        with Pool(n_pool) as pool:
            ...
            new_columns = list(itertools.chain.from_iterable(
                [['%s_lag_%s_mean' % (col, lag), '%s_lag_%s_max' % (col, lag), '%s_lag_%s_min' % (col, lag)]
                 for col, lag in zip(self.LAG_FEATURES, self.lags)]))
        df = df.fillna({col: -1 for col in new_columns}).drop(["time", "assetCode"], axis=1)

        if self.scale:
            if not self.scaler:
                self.scaler = {col: MinMaxScaler().fit(df[col].values.reshape((-1, 1))) for col in new_columns}
            for col in new_columns:
                df[col] = self.scaler[col].transform(df[col].values.reshape((-1, 1))).astype("float32")
    ...
    def extract_lag(self, asset_code_group):
        ...
        return asset_code_group.fillna(-1)
